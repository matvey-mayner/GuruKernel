...
local installerVersion = "GuruKernel Setuper"
...
local function selectDist(dists)
    ...
    while true do
        ...
        if funcs[num] then
            ...
            if proxy then
                if menu("Comfirm? Init.lua will be rewrited!", {"No!", "OK"}) == 2 then
                    status("Please Wait")
    ...
    for i, v in ipairs(filelist) do
        if v ~= "" then
            ...
            table.insert(dists, {name = name, call = function(proxy)
                download("https://raw.githubusercontent.com/matvey-mayner/GuruKernel/main", proxy)
                download(url, proxy)
            end})
        end
    end

    table.insert(dists, {name = "Install Kernel", call = function(proxy)
        download("https://raw.githubusercontent.com/matvey-mayner/GuruKernel/main", proxy)
    end})
...
if internet then
    table.insert(strs, 1, "online mode (!Only Online!)")
